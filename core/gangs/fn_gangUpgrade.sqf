...
if(_action) then {
	if(man_ey_b4nK < _upgradePrice) exitWith {
		hint parseText format[
			...
			[man_ey_b4nK] call life_fnc_numberText,
			[_upgradePrice - man_ey_b4nK] call life_fnc_numberText
		];
	};
	__SUB__(man_ey_b4nK,_upgradePrice);
